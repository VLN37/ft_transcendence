...
export class Rect {
    ...
    squarify(data: number[], x: number, y: number, w: number, h: number): Rect[] {
        let ratio = (w: number, h: number) => { return w > h ? w / h : h / w };
...
export class Voronoi {
    ...
    compute(points: Point[]) {
        ...
        for (let i = 0; i < points.length; i++) {
            let p = points[i];
            (<any>p).__circumcenters.sort(function (a: Point, b: Point) {
...
export class LimitedVoronoi {
    ...
    compute(cells: Cell[]|Circle[]) {
        ...
        function intersection(s1: any, s2: any) {
            let d = s1.p.distance(s2.p);
        ...
        for (let i = 0; i < sites.length; i++) {
            ...
            s.__points.sort(function (a: any, b: any) {
...
export class Cell {
    ...
    r: number = 1;
    vertices: CellVertex[];
    state: number = 0; //disjoint, intersect, contain
